      ...
      <div className="relative w-full max-w-sm bg-white h-full shadow-[-20px_0_80px_rgba(0,0,0,0.4)] flex flex-col animate-in slide-in-from-right duration-500 border-l border-zinc-200">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {loading ? (
                ...
                activities.map((act) => (
                    <div key={act.id} className={`p-5 rounded-2xl border transition-all flex gap-4 ${act.is_read ? 'bg-transparent border-transparent opacity-60' : 'bg-blue-50/50 border-blue-100'}`}>
                        <div className="shrink-0">
                            {act.actor?.avatar_url ? (
                                <Image src={act.actor.avatar_url} alt="actor" width={40} height={40} className="w-10 h-10 rounded-2xl object-cover border-2 border-white shadow-sm" />
                            ) : (
                                <div className="w-10 h-10 bg-zinc-100 rounded-2xl flex items-center justify-center text-zinc-400"><User size={20} /></div>
                            )}
                        </div>
  ...
  return (
    <>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 flex h-5 min-w-[20px] px-1 items-center justify-center bg-red-500 text-white text-[10px] font-bold rounded-full border-2 border-white animate-in zoom-in">
